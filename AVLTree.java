...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        // 1. Check if node passed as a param is null, if so then:
        if (node == null)
            return null; // tree is empty or reached null subtree

        // 2. check if we need to traverse the left or right child.
        // a. recursively call deleteElement() along proper subtree.
        if (value > node.value) {
            node.rightChild = deleteElement(value, node.rightChild);
        } else if (value < node.value) {
            node.leftChild = deleteElement(value, node.leftChild);
        } else {
            // the current node equals the value
            // 4. If value found, we are at proper node to delete:
            if (node.leftChild == null && node.rightChild == null) {
                // a. if no sub-children, return null (this delete’s node)
                return null; // leaf node so we return null (setting left or right child to null)
            } else if (node.leftChild == null) {
                // b. if only one subtree: Assign approp subtree to this node(aka, move subtree
                // up)
                return node.rightChild; // only right child, so we return the right child
            } else if (node.rightChild == null) {
                return node.leftChild; // only left child, so we return the left child
            } else {
                // c. If left AND right subtree
                // inorder successor node (aka, the smallest value node in the right subtree)
                // i. Find in-order successor node (via minValueNode())
        ...
        // a. Recalculate node’s height.
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
        // b. Recalculate node’s bf via getBalance()
        int balanceFactor = getBalanceFactor(node);
        // 6. If this node’s |bf| > 1, then rotation needed for this node:
        if (Math.abs(balanceFactor) > 1) {
            // a. Invoke LL, LR, RR, or RL rotation.
